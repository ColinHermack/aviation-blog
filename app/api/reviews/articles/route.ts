import { ArticleMetadata, getPaginatedReviewMetadata } from "@/articles/utils";
...
export function GET(request: Request): Response {
    try {
        if (request.headers.get("page") === null) {
            ...
            return new Response(JSON.stringify(getPaginatedReviewMetadata(parseInt(request.headers.get("page")!))), {status: 200});
        }
    } catch (error) {
